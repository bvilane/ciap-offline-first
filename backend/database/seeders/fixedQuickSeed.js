...
// ============================================
// STEP 2: INSERT DATA (with deduplication)
// ============================================
function insertData(table, data, uniqueColumn = 'id') {
  return new Promise((resolve, reject) => {
...
async function seed() {
  try {
    ...
    const communities = [
      { slug: 'acornhoek', name: 'Acornhoek', created_at: now },
      { slug: 'hoedspruit', name: 'Hoedspruit', created_at: now },
      { slug: 'mbombela', name: 'Mbombela', created_at: now }
    ];

    for (const community of communities) {
      await insertData('communities', community, 'slug');
    }
    ...
    for (const user of users) {
      await insertData('users', user, 'email');
    }
    ...
    console.log('   User: test@ciap.local / Test123!');
