...
router.get('/', async (req, res, next) => {
  try {
    ...
    let orderBy = 'ORDER BY COALESCE(starts_at, created_at, 0) DESC';
    if (sort === 'title') {
    ...
    const sql = `SELECT * FROM skills ${whereClause} ${orderBy} LIMIT ? OFFSET ?`;
...
router.post('/', async (req, res, next) => {
  try {
    ...
    const result = await db.execute(
      `INSERT INTO skills (community, title, provider, summary, url, starts_at, featured, status, created_at)
       VALUES (?,?,?,?,?,?,?,?,?)`,
      [community, title, provider, summary, url, startsAt ? Number(startsAt) : null, featured, 'pending', now]
    );
